...
test('M7-01 garde ses preuves historiques distinctes du correctif réseau v2.22.2', () => {
    ...
    assert.equal(layout.match(/APP_VERSION\s*=\s*['"]([^'"]+)['"]/u)?.[1], 'v2.22.2');
    assert.equal(sw.match(/APP_VERSION\s*=\s*['"]([^'"]+)['"]/u)?.[1], 'v2.22.2');
    assert.match(app, /app-version"\s+content="v2\.22\.2"/u);
    assert.match(sw, /CACHE_NAME\s*=\s*['"]wfrp-cache-['"]\s*\+\s*APP_VERSION/u);
    assert.match(changelog, /^## \[2\.22\.2\]/mu);
