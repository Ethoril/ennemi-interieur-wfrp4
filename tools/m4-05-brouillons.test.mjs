...
test('la release M4-05 aligne meta/cache/changelog et garde /app hors annonce', () => {
    ...
    assert.match(html, /app-version" content="v2\.21\.1"/u);
    assert.match(layout, /APP_VERSION = 'v2\.21\.1'/u); assert.match(sw, /APP_VERSION = 'v2\.21\.1'/u);
    assert.match(changelog, /^## \[2\.21\.1\]/mu);
