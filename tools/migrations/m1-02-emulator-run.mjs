// Runner Emulator Suite M1-02 : aucune cible de production n'est acceptée.
...
if (project === 'campagne-wrpg' || process.env.GCLOUD_PROJECT === 'campagne-wrpg'
    ...
    console.error('Runner M1-02 refusé : cible de production interdite');
    process.exit(1);
}
...
if (process.env.M1_EMULATOR_CHILD === '1') {
    if (!process.env.FIRESTORE_EMULATOR_HOST) {
        console.error('Runner M1-02 refusé : FIRESTORE_EMULATOR_HOST absent');
        process.exit(1);
    }
    await import('./m1-02-emulator.test.mjs');
} else {
    const temp = await mkdtemp(join(tmpdir(), 'm1-02-emulator-'));
    ...
    try {
        result = spawnSync(process.execPath, [cli, 'emulators:exec', '--only', 'firestore', '--project', project,
            '--config', configPath, `node "${resolve(root, 'tools/migrations/m1-02-emulator.test.mjs')}"`], {
            cwd: root,
    ...
    if (result.error || result.status !== 0) {
        console.error(`Emulator Suite M1-02 échouée (code ${result.status ?? 'inconnu'}).`);
