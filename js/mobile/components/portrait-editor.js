...
export async function processPortraitFile(file, options = {}) {
    ...
    try {
        ...
        const target = Math.max(1, Math.min(MAX_DIMENSION, Math.round(side)));
        canvas = canvasFor(options);
        canvas.width = target; canvas.height = target;
        const context = canvas.getContext?.('2d', { alpha: false });
        if (!context?.drawImage) throw invalid('Le traitement du portrait est indisponible.');
        const sx = Math.max(0, (width - side) / 2); const sy = Math.max(0, (height - side) / 2);
        ...
        return Object.freeze({ blob: output, contentType: output.type || outputType, width: target, height: target,
...
export function createPortraitEditor({ container, document: documentRef = container?.ownerDocument ?? globalThis.document,
    ...
    const legend = documentRef.createElement('legend'); legend.textContent = 'Portrait'; portraitFieldset.append(legend);
    ...
    const help = documentRef.createElement('span'); help.id = 'm-portrait-help'; help.className = 'm-form-help'; help.textContent = 'Photo ou photothèque, source 20 Mo maximum, portrait final 2 Mo. Cadrage carré centré ; la caméra nécessite HTTPS et une autorisation.';
    ...
    const preview = documentRef.createElement('img'); preview.alt = 'Aperçu du portrait'; preview.hidden = true; preview.className = 'm-portrait-preview';
    const weight = documentRef.createElement('span'); weight.className = 'm-portrait-weight';
    const remove = documentRef.createElement('button'); remove.type = 'button'; remove.className = 'm-button'; remove.textContent = 'Retirer le portrait';
    ...
    const select = async sourceInput => {
        ...
        try {
            ...
            catch { throw invalid('Aperçu du portrait indisponible.', 'unknown'); }
            ...
            status.textContent = 'Portrait prêt à être enregistré.'; update();
        } catch (error) {
            if (local !== generation || localController.signal.aborted) return;
            status.textContent = error?.message || 'Portrait invalide.'; announce(status.textContent); sourceInput.value = '';
        } finally {
    ...
    remove.addEventListener('click', () => {
        ...
        if (typeof documentRef.defaultView?.confirm === 'function' && !documentRef.defaultView.confirm('Retirer le portrait lors de l’enregistrement ?')) return;
        generation += 1; controller?.abort(); processing = false; selected = null; removalRequested = true; revoke(); cameraInput.value = ''; libraryInput.value = ''; weight.textContent = ''; status.textContent = 'Le portrait sera retiré à l’enregistrement.'; update();
    });
    return Object.freeze({
        ...
        setCurrentPath: async (path, imageService) => {
            ...
            try {
                ...
                currentHandle = loadedHandle; preview.src = currentHandle.url; preview.hidden = false; status.textContent = 'Portrait actuel. Cadrage carré centré ; choisissez un fichier pour le remplacer.';
